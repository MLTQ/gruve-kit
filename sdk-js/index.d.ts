...
/**
 * A namespaced BroadcastChannel for talking to other Gruve apps on the same machine. Gruve serves
 * all apps from one dedicated origin (isolated from the lobby), so they can coordinate via
 * BroadcastChannel / localStorage. This is a thin convenience over `new BroadcastChannel(...)`.
 */
export function appChannel(name?: string): BroadcastChannel
