...
/**
 * Talk to other Gruve apps running on the same machine.
 *
 * Gruve serves every app from a single dedicated origin (separate from the lobby, so apps can't
 * touch its controls or secrets — but shared *between apps*). That means apps on one machine can
 * coordinate the way same-origin pages always can: a BroadcastChannel, or window.localStorage.
 *
 * `appChannel(name)` returns a namespaced BroadcastChannel so two apps can agree on a topic without
 * colliding with the page's own channels. It's a thin convenience — `new BroadcastChannel(...)` and
 * `localStorage` work directly too.
 *
 * @example
 ...
 *
 * @param {string} name  a topic shared by the cooperating apps
 * @returns {BroadcastChannel}
 */
export function appChannel(name = 'default') {
  if (typeof BroadcastChannel === 'undefined') {
    throw new Error('gruve appChannel: BroadcastChannel is not available in this environment')
  }
  return new BroadcastChannel('gruve.app:' + name)
}
