...
## 7. Apps run on their own origin — isolated from the lobby, shared with each other

Gruve serves every app from a **dedicated origin** (a separate local port from the lobby). Two
consequences, both deliberate:

- **You're isolated from Gruve itself.** Your app can't reach the lobby's control API (`/api/*`),
  read its network secrets, or touch its DOM — those live on a different origin, and the agent
  rejects any cross-origin attempt. So a buggy or hostile app can't delete someone's networks or
  leak their chat keys. (The flip side of "vibecoded apps run safely.")
- **You're same-origin with other Gruve apps on that machine — use it.** Apps share the apps
  origin, so they can coordinate the way same-origin pages always can: `localStorage`,
  `BroadcastChannel`, etc. `appChannel(name)` is a thin namespaced helper:

  ```js
  import { appChannel } from 'gruve-sdk'
  const ch = appChannel('now-playing')
  ch.postMessage({ track: 'ETH/USDT' })   // a music app announces
  ch.onmessage = (e) => render(e.data)    // a visualizer app reacts
  ```

  This is local, machine-scoped coordination (one person's apps talking to each other), distinct
  from `joinSession`, which is the cross-mesh room shared with everyone viewing your app.
